function [eval] = GC_getBoardEvalBetter(board, color, moveNo)
...
%% FRONSTEINE: Nachbarn z�hlen (imfilter() nachgebaut)
    ...
    frontM = zeros(size(b_all));    % prealloc
    filt = [ 1 1 1; 1 0 1; 1 1 1];
    numMin = 8;    
    % replicate edges (vital!)
    b_all = [ b_all(1,:); 
              b_all(:,:); 
              b_all(end,:)];
    b_all = [b_all(:,1) b_all(:,:) b_all(:,end)];

    % 8x8 spielfeld
    for xx = 2:9
            for yy=2:9
                tmp=b_all(yy-1:yy+1, xx-1:xx+1) .* filt;
                frontM(yy-1,xx-1)=sum(tmp(:));
            end
    end
