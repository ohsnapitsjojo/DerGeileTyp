function [ v, m ] = G4_alphaBetaPruning( b, depth, alpha, beta, color, turn, w, toplayer )

    m = [];
    possMoves = G4_allPossible(b, turn);
    nMoves = length(possMoves);

    % Wenn Leaf, dann Heuristic zurückgeben
    if depth == 0|| nMoves== 0
        v = G4_getHeuristic( b, w, color );
        % TODO: HEURISTIC HIER EINSETZEN
        return 
    end

    % Wenn Max Layer, dann v Werte berechnen und mit Alpha Werte
    % abgleichen. Größerer Wert wird übernommen
    if ((turn == color  && nMoves~=0)||(turn== -color && nMoves==0))
       v = -inf;

       for k=1:nMoves
           bNew = G4_simulateMove(b, turn, possMoves(k));
           [vNew, ~] = G4_alphaBetaPruning(bNew, depth-1, alpha, beta, color, turn*-1, w, false);
           v = max(v, vNew);
           if alpha < v
              alpha = v;
              if toplayer == true
                % Wenn toplayer und Alpha wert wird upgedatet -> Zug
                % zurückgeben.
                m = possMoves(k);
              end
           end

           if beta <= alpha
               % Ast musst nicht weiter betrachtet werden sobald 
               % beta <= alpha
              break;
           end

       end
       return
    else
        v = inf;
        
        for k=1:nMoves
           bNew = G4_simulateMove(b, turn, possMoves(k));
           [vNew, ~] = G4_alphaBetaPruning(bNew, depth-1, alpha, beta, color, turn*-1, w, false);
           v = min(v, vNew);
           if beta > v
              beta = v;
              if toplayer == true
                % Wenn toplayer und Beta wert wird upgedatet -> Zug
                % zurückgeben.
                m = possMoves(k);
              end
    
           end

           if beta <= alpha
               % Ast musst nicht weiter betrachtet werden sobald 
               % beta <= alpha
              break;
           end

           
       end
       return
    end
end

